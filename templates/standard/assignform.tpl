...
<label for="projects{$projects[proj].ID}">{$projects[proj].name}:</label><input type="checkbox" value="{$projects[proj].ID}" id="projects{$projects[proj].ID}" name="projects[]" required="0" {if $projects[proj].assigned == 1}checked{/if} /><br />
{/section}
