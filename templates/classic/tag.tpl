				...
				<div class="table_body">
					...
					<div id="results_focus{$result[obj].ID}" class="focus_off">
							...
							{if $result[obj].type == "file"}
								<a style="top:-33px;" href = "files/{$cl_config}/{$result[obj].project}/{$result[obj].name}" {if $result[obj].imgfile == 1} rel="lytebox[]" {elseif $result[obj].imgfile == 2} rel = "lyteframe[text]" rev="width: 650px; height: 500px;" {/if}>{$result[obj].name|truncate:40:"...":true}</a>
							...
							<td class="a"></td>
							{if $result[obj].desc != ""}
							<td class="descript">{$result[obj].desc|nl2br}
							{else}
							<td class="descript">{$result[obj].text|nl2br}
							{/if}
							{if $result[obj].tagsarr[0] != ""}<br /><br /><strong>{#tags#}:</strong>
							{section name = tag loop=$result[obj].tagsarr}
							<a href = "managetags.php?action=gettag&tag={$result[obj].tagsarr[tag]}&amp;id={$project.ID}">{$result[obj].tagsarr[tag]}</a>
							{/section}
							{/if}
							</td>
							</tr>
